...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain category"
      decision: "missing capability — skill gap, not workflow/data/coordination"
    - topic: "insight"
      decision: "flashcards make practice bite-sized and habitual, vs. long-form eloquence books/courses"
    - topic: "primary persona scope"
      decision: "general young adults in social settings (conversations with older/more articulate acquaintances, family, mentors)"
    - topic: "access model"
      decision: "local profile, data on-device, no login/server; flat single-user model"
    - topic: "MVP flow"
      decision: "open app -> input description of person/interests -> tailored vocabulary flashcard -> mark known/unknown -> next card; 1.5-week timeline, no scope-down needed"
    - topic: "product framing"
      decision: "mobile app, small scale (just me / handful), hard deadline 2026-08-27, after-hours only. Scale probe: matching logic holds at 100x since it's per-user/local, no shared component."
    - topic: "non-goals"
      decision: "no custom-trained NLP/matching model — simple/rule-based word selection only"
  frs_drafted: 4
